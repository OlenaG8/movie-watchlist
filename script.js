...
document.addEventListener("click", (e) => {
    if (e.target.matches('#search-btn')) {
        ...
        addToOrRemoveFromWatchlist(movieId)
    }
})
...
function addToOrRemoveFromWatchlist(id) {
    const movieIndex = myWatchlist.findIndex(watchlistMovie => watchlistMovie.imdbID === id)
    if (movieIndex === -1) {
        addToWatchlist(id)
    } else {
...
function addToWatchlist(id) {
    const movie = moviesByIdMap.get(id)
